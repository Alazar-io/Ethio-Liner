...
def generate_ticket_signature(ticket_reference: str, trip_id: int, seat_number: str) -> str:
    """Generates an HMAC-SHA256 signature for tamper-proofing digital QR tickets."""
    message = f"{ticket_reference}:{trip_id}:{seat_number}".encode("utf-8")
    sig = hmac.new(settings.SECRET_KEY.encode("utf-8"), message, hashlib.sha256).digest()
    return base64.urlsafe_b64encode(sig[:12]).decode("utf-8").rstrip("=")


def verify_ticket_signature(ticket_reference: str, trip_id: int, seat_number: str, signature: str) -> bool:
    """Verifies that the ticket signature was authentically generated by the server."""
    expected = generate_ticket_signature(ticket_reference, trip_id, seat_number)
